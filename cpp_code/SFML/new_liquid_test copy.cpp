		...
		double Calculate_density(Particle * particles, vector2D<double> pos)
			{
				...
				for(int ii=0; ii<N_particles; ii++)
					{density+=particles[ii].mass*smoothing_kernel(particles[ii].pos-pos);}
		...
		double Calculate_property(Particle * particles, vector2D<double> pos)
			{
				...
				for(int ii=0;ii<N_particles;ii++)
					{	
						property+=particles[ii].property*smoothing_kernel(particles[ii].pos-pos)*
		...
		vector2D<double> Calculate_preasure_gradient(Particle * particles, int particle_index)
			{
				...
				for(int ii=0;ii<N_particles;ii++)
					{	
						...
						preasure_gradient+= - particles[ii].preasure*smoothing_kernel_gradient(particles[ii].pos-pos)*
						particles[ii].mass/particles[ii].density;
					}
...
int main()
	{
		...
		while (window.isOpen())
			{	
				...
				std::cout<<"mid_density: "<<particles[mid_point].density<<"\n";
				if(show_influence)
					{	
						// std::cout<<"mid_point: "<<mid_point<<"\n";
